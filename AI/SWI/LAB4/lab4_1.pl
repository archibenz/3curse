...
colussi_file(Input, Output) :-
    read_strings_from_file(Input, Pattern, Text, Extra),
    kmp_search_all(Pattern, Text, Positions),
    open(Output, write, Stream),
    write(Stream, Positions),
    nl(Stream),
    close(Stream),
    ( Extra == true ->
        writeln('Предупреждение: во входном файле были лишние строки, они были проигнорированы.')
    ; true ).
